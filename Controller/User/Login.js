...
export async function Login(req, res) {
    ...
    if (!email) {
        return res.status(400).send({ success: false, error: "Email are required!" });
    }
    if (!password) {
        return res.status(400).send({ success: false, error: "Password are required!" });
    }
    try {
        ...
        res.status(500).send({ success: false, error: "Server error, please try again later." });
    }
}
...
//         if (!payload) return res.status(401).json({ success: false, error: "Invalid ID token" });
//         const { id: googleId, email, name, picture } = payload;
//         if (!googleId) {
//             return res.status(400).json({ success: false, error: "Invalid Google ID" });
//         }
